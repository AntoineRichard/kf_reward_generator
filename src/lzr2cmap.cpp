...
#include <numeric>
#include <functional>
#include "reward_generator/reward_generator.h"

//#define DEBUG
//#define SHOW_IMG

LZR2CMap::LZR2CMap() : nh_("~"), it_(nh_) {
    // from launch
  	std::string laser_frame("front_laser");
  	nh_.param("dist_to_shore", dist_to_shore_, 5.0f);
  	nh_.param("safe_to_shore", safe_to_shore_, 2.0f);
  	nh_.param("laser_frame", lzr_frame_, laser_frame);
  	nh_.param("local_grid_front_size", local_grid_front_size_, 10.0f);
  	nh_.param("local_grid_rear_size", local_grid_rear_size_, 2.0f);
  	nh_.param("local_grid_side_size", local_grid_side_size_, 6.0f);
  	nh_.param("local_grid_res", local_grid_res_, 0.1f);
    ...
    lzr_sub_ = nh_.subscribe("laser",1,&LZR2CMap::LZRCallback,this);
    refresh_sub_ = nh_.subscribe("refresh",1,&LZR2CMap::refreshCallback,this);
    done_sub_ = nh_.subscribe("done",1,&LZR2CMap::doneCallback,this);
    odom_sub_ = nh_.subscribe("odom",1,&LZR2CMap::odomCallback,this);
	  reward_pub_ = nh_.advertise<std_msgs::Float32>("reward",1,true);
    image_pub_ = it_.advertise("DreamersView", 1);
 
}

void LZR2CMap::LZRCallback(const sensor_msgs::LaserScanConstPtr& data) {
    if (compute_) {
        refreshMap();
        buffer_.clear();
        inner_shores_.clear();
...
void LZR2CMap::odomCallback(const nav_msgs::OdometryConstPtr& data) {
    ...
    if (vlin > 0) {
        vel_reward_ = (vel_target_*vel_target_ - (vel_target_ - vlin)*(vel_target_ - vlin))*vel_coeff_/(vel_target_*vel_target_);
        /*if (vlin < vel_target_) {
            vel_reward_ = (vlin/vel_target_)*vel_coeff_;
        } else {
            vel_reward_ = (vel_target_ - fabs(vel_target_-vlin))*vel_coeff_;
        }*/
    } else {
        vel_reward_ = backward_coeff_;
    }
    //ROS_INFO("reward %.3f",vel_reward_);
}

void LZR2CMap::doneCallback(const std_msgs::BoolConstPtr& data) {
    compute_ = false;
    ROS_INFO("Pausing laser processing");
}

void LZR2CMap::refreshCallback(const rl_server::EpisodeConstPtr& data) {
    compute_ = true;
    ROS_INFO("Resuming laser processing...");
}

void LZR2CMap::initializeMap() {
    // Instantiate maps
  	if((kernel_size_ & 1) == 0){
  		kernel_size_ ++;
  	}
...
void LZR2CMap::setMapBlockValue(int cx, int cy, cv::Mat& the_kernel, int ksize, cv::Mat& the_map, cv::Scalar val) {
    /*
...
void LZR2CMap::genCostMap(){
    ...
    //cropped_map_.setTo(0,cropped_map_ < 2);
    //ROS_INFO("map width %d, height %d",cost_map_.cols, cost_map_.rows);
    //ROS_INFO("cropped map width %d, height %d",cropped_map_.cols, cropped_map_.rows);
    //ROS_INFO("ranges width: start: %d, end: %d, height: start: %d, end: %d",cropw_.start, cropw_.end, croph_.start, croph_.end);
    //cv::Mat_<float> tmp;
    //cv::threshold(cropped_map_, tmp, 255, 255, cv::THRESH_TRUNC);
    //tmp.convertTo(n_cost_map_, CV_8U, 1.0);
    //cv::cvtColor(n_cost_map_, img_net_, cv::COLOR_GRAY2RGB);
    //int xpos = (int) 32;//(64/hsize_)*local_grid_rear_size_;
    //int ypos = (int) 11;//(64/wsize_)*local_grid_side_size_;
    //ROS_INFO("cost: %.3f",cropped_map_.at<float>(20, 120));
    //ROS_INFO("cost: %.3f",cropped_map_.at<float>(x_offset_, y_offset_));
    //ROS_INFO("cost: %.3f",cropped_map_.at<float>(y_offset_, x_offset_));
    //pose_reward_ = std::max(-10.,(1. - cropped_map_.at<float>(x_offset_, y_offset_))*pose_coeff_);
    //ROS_INFO("pose reward: %.3f", pose_reward_);
    pose_reward_ = std::max(-20.0, (1. - cropped_map_.at<float>(y_offset_, x_offset_)*0.25)*pose_coeff_);
    //ROS_INFO("pose reward: %.3f", pose_reward_);
}

// Publish occupancy grid for rviz
void LZR2CMap::publishOccupancyGrid() {
...
void LZR2CMap::publishReward() {
    reward_.data = pose_reward_ + vel_reward_;
    //ROS_INFO("pose reward: %.3f, vel reward: %.3f", pose_reward_, vel_reward_);
	  reward_pub_.publish(reward_);
}    

// Takes the laser outputs and store them into a buffer.
void LZR2CMap::addToBuffer(const sensor_msgs::LaserScanConstPtr & data) {
    float xf, yf;
	  int x, y;
...
void LZR2CMap::filterPoints(PointsVector& in_v, PointsVector& out_v) {
#ifdef DEBUG
...
//Interpolates points in the vector, in case of a gap in the curve. 
void LZR2CMap::interpolatePoints(PointsVector& in_v, PointsVector& out_v) {
    /*
...
double LZR2CMap::calcDist(const PointsVector& pts, size_t i, size_t j){
